...
##### 
ABC_liger <- merge(sampleA1, y = c(sampleB1, sampleC1))

#####
ABC_liger <- NormalizeData(ABC_liger, verbose = T)
ABC_liger <- FindVariableFeatures(ABC_liger, verbose = T)
ABC_liger <- ScaleData(ABC_liger, verbose = T, split.by = "orig.ident", do.center = F)

#####
ABC_liger <- RunOptimizeALS(ABC_liger, k = 30, lambda = 5, split.by = "orig.ident") ## this one takes a while

#####
ABC_liger <- RunQuantileNorm(ABC_liger, split.by = "orig.ident")

#####
ABC_liger <- FindNeighbors(ABC_liger,reduction = "iNMF", k.param = 10, dims = 1:30)
ABC_liger <- FindClusters(ABC_liger)

#####
ABC_liger <- RunUMAP(ABC_liger, dims = 1:ncol(ABC_liger[["iNMF"]]), reduction = "iNMF", verbose = F)
# ABC_liger <- SetIdent(pbmc_liger,value = "orig.ident")
DimPlot(ABC_liger, reduction = "umap", group.by = "orig.ident") + plot_annotation(title = "ABC, after integration (LIGER)")
DimPlot(ABC_liger, reduction = "umap", group.by = "orig.ident", split.by = "orig.ident") + plot_annotation(title = "ABC, after integration (LIGER)") + NoLegend()

DimPlot(ABC_liger, reduction = "umap", group.by = "ident", label = T, split.by = "orig.ident") + plot_annotation(title = "ABC, after integration (LIGER)")
...
count_table <- table(ABC_liger@meta.data$seurat_clusters, ABC_liger@meta.data$orig.ident)
...
plot_integrated_clusters(ABC_liger) 

# #### Puis sur les lames?
# SpatialDimPlot(ABC_liger, ncol = 2)
# SpatialDimPlot(ABC_liger, ncol = 3)
SpatialPlot(ABC_liger, group.by = "ident")
SpatialDimPlot(ABC_liger, ncol = 3, images = "sliceA1",
               cells.highlight = CellsByIdentities(object = ABC_liger, idents = c(0, 2, 3, 4, 6, 7, 10, 11, 12)),
               facet.highlight = TRUE)
SpatialDimPlot(ABC_liger, ncol = 3, images = "sliceB1",
               cells.highlight = CellsByIdentities(object = ABC_liger, idents = c(0, 1, 2, 5, 6, 7, 11, 15, 16)),
               facet.highlight = TRUE)

SpatialDimPlot(ABC_liger, ncol = 3,
               cells.highlight = CellsByIdentities(object = ABC_liger, idents = c(0)))
# SpatialDimPlot(ABC_liger, ncol = 3,
#                cells.highlight = CellsByIdentities(object = ABC_liger, idents = c(1, 2)), facet.highlight = TRUE)

# saveRDS
saveRDS(ABC_liger, file = "output/ABC_object/20220622_ABC_object_method3.rds")
...
# split the object
split.data <- SplitObject(ABC_liger, split.by = "orig.ident")
...
write.table(split.umap, file = "output/correctedABC/20220622_method3_corrected_umap_ABC.csv", sep = ",", quote = F, row.names = F, col.names = T)
...
write.table(split.cluster, file="output/correctedABC/20220622_method3_corrected_clusters_ABC.csv", sep = ",", quote = F, row.names = F, col.names = T)