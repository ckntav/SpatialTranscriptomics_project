...
A1B1C1 <- NormalizeData(A1B1C1) %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA(verbose = TRUE)
DimPlot(A1B1C1, group.by = "orig.ident")
A1B1C1 <- RunUMAP(A1B1C1, dims = 1:30)
DimPlot(A1B1C1, group.by = "orig.ident")

##### 5. Run Harmony
A1B1C1 <- RunHarmony(A1B1C1, group.by.vars = "orig.ident")
A1B1C1 <- RunUMAP(A1B1C1, reduction = "harmony", dims = 1:30)
A1B1C1 <- FindNeighbors(A1B1C1, reduction = "harmony", dims = 1:30) %>% FindClusters()
...
DimPlot(A1B1C1, group.by = c("orig.ident", "ident"))
...
write.table(corrected.umap, file = "output/correctedA1B1C1/20220620_f_corrected_umap_A1B1C1.csv", sep = ",", quote = F, row.names = F, col.names = T)
...
write.table(corrected.cluster, file="output/correctedA1B1C1/20220620_f_corrected_clusters_A1B1C1.csv", sep = ",", quote = F, row.names = F, col.names = T)
