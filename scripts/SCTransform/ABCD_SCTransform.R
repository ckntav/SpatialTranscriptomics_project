...
sampleB1 <- SCTransform(sampleB1, assay = "Spatial", verbose = TRUE)
sampleC1 <- SCTransform(sampleC1, assay = "Spatial", verbose = TRUE)
sampleD1 <- SCTransform(sampleD1, assay = "Spatial", verbose = TRUE)

#
ABCD <- merge(sampleA1, y = c(sampleB1, sampleC1, sampleD1))

#
VariableFeatures(ABCD) <- c(VariableFeatures(sampleA1), VariableFeatures(sampleB1), VariableFeatures(sampleC1), VariableFeatures(sampleD1))
ABCD <- RunPCA(ABCD, verbose = FALSE)
DimPlot(ABCD, reduction = "pca", group.by = c("orig.ident"))
ABCD <- FindNeighbors(ABCD, dims = 1:30)
ABCD <- FindClusters(ABCD, verbose = FALSE)
ABCD <- RunUMAP(ABCD, dims = 1:30)

DimPlot(ABCD, reduction = "umap", group.by = c("ident", "orig.ident"))
SpatialDimPlot(ABCD)
